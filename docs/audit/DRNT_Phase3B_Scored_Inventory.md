# DRNT Phase 3B — Scored Claims Inventory

## Preamble

- Orchestration HEAD: `47e1b9f890861942129bdfd6484c5e7ec8b7ee33` (unchanged from 3A)
- Gateway HEAD: `6e9dc2271c2726d1d564bb0aa946142b69b99d6b` (unchanged from 3A)
...
- Working trees at scoring time: clean (orchestration has untracked `docs/audit/`)
- Generation date: 2026-04-19
- Posture: read-only; observation of code vs. claim; no spec or code edits; no cross-referencing between specs
- Primary evidence source: `DRNT_Phase1_Phase2_Findings_Inventory.md` (Phase 1 + Phase 2). Secondary: gateway code-read.
...
- Scoring taxonomy (five states): **aligned** — findings and/or code-read directly support the claim as stated; **partial** — claim holds in part but materially diverges in specifics (scope, timing, emission, wiring); **missing** — no implementation evidence found (no code path or emitter); **contradicted** — findings and/or code-read directly contradict the claim; **pending-cross-ref** — resolution requires cross-spec correlation, unprobed behavior, absent artifacts, or governance/policy-layer judgment not testable from code alone (deferred to 3C). The 18 3A-flagged ambiguous claims default to `pending-cross-ref` unless findings + local code-read resolved them cleanly; claims whose resolution depends on observing iOS/Watch/MacBook layers (outside gateway repo) or on unprobed runtime behavior are `pending-cross-ref` rather than missing, since code-side implementation may still exist.

---

## Summary

| Spec | Total | aligned | partial | missing | contradicted | pending-cross-ref |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 — Audit/Event Schema | 94 | 60 | 10 | 1 | 2 | 21 |
| 2 — Capability Model | 86 | 55 | 19 | 4 | 0 | 8 |
| 3 — Context Boundary | 38 | 18 | 3 | 1 | 0 | 16 |
| 4 — Egress Policy | 39 | 25 | 5 | 0 | 0 | 9 |
| 5 — Override Semantics | 40 | 13 | 6 | 0 | 0 | 21 |
| 6 — Silo Runtime Security | 114 | 37 | 17 | 14 | 19 | 27 |
| 7 — Signal Chain Resilience | 103 | 26 | 17 | 2 | 6 | 52 |
| **Total** | **514** | **234** | **77** | **22** | **27** | **154** |
...
| S1-C27 | §3 | structure | `capability_id` is null for system events and `job.submitted` (pre-classification). | (none) | pending-cross-ref | 3A pre-flag (S1-C27/C28 transition rule needs testing against actual events) |
| S1-C28 | §3 | behavior | `capability_id` is set at `job.classified` and carried through all subsequent events; never mutated. | §3.1 | pending-cross-ref | 3A pre-flag |
...
| S1-C35 | §3.1 | behavior | Auxiliary capabilities emit their own `wal.permission_check` events; envelope always reflects governing. | Spec 2 §2.1 | pending-cross-ref | Depends on Spec 2 §2.1 |
| S1-C36 | §3.1 | structure | Auxiliary checks carry `dependency_mode`: `required` │ `optional` │ `best_effort`. | Spec 2 §2.1 | pending-cross-ref | Depends on Spec 2 §2.1 |
...
| S1-C41 | §3.1 | behavior | Redirect and escalate overrides terminate the original and spawn a successor. | Spec 5 | pending-cross-ref | Depends on Spec 5 |
...
| S1-C45 | §4.1 | behavior | `job.failed` may be followed by a new `job.dispatched` with incremented `retry_sequence`. | (none) | pending-cross-ref | FI §4 (retry path not exercised) |
| S1-C46 | §4.1 | state | v1 retry constraint: same capability only. | Spec 2 §8 | pending-cross-ref | Depends on Spec 2 §8 |
| S1-C47 | §4.1 | structure | Redirect and escalate spawn a successor with a new `job_id`. | Spec 5 | pending-cross-ref | Depends on Spec 5 |
...
| S1-C50 | §4.1 | behavior | `job.dispatched` emitted when gateway confirms outbound transmission, BEFORE model response. | Spec 4 §4 | pending-cross-ref | Depends on Spec 4 §4 |
...
| S1-C57 | §4.2 | behavior | Rollup arrays use `field_id`s, not names; a single `field_id` may appear in multiple rollup arrays. | (none) | pending-cross-ref | Deep payload structure; not exercised |
...
| S1-C61 | §4.3 | invariant | `delivery_hold: true` on allowed result → `pre_delivery` gate applied downstream. | Spec 2 §4 | pending-cross-ref | Depends on Spec 2 §4 |
| S1-C62 | §4.3 | invariant | `pre_delivery` is never a `held` result. | (none) | pending-cross-ref | Depends on Spec 2 §4 |
...
| S1-C65 | §4.4 | behavior | Single-model vs multi-model `selected: true` semantics. | (none) | pending-cross-ref | 3A pre-flag |
...
| S1-C67 | §4.4.1 | structure | Structured responses hashed as UTF-8 canonical JSON. | (none) | pending-cross-ref | Deep serialization detail |
...
| S1-C69 | §4.5 | structure | Modified result lineage fields on `human.reviewed`. | (none) | pending-cross-ref | Depends on Spec 5 |
...
| S1-C73 | §6.1 | state | V1 retention: retain all log files indefinitely. | (none) | pending-cross-ref | Operational; no retention policy code |
| S1-C74 | §6.2 | state | V1 querying tooling: `jq`, `grep`, `tail -f`. No database. | (none) | pending-cross-ref | Operational/external tooling |
...
| S1-C76 | §6.3 | invariant | Modified results stored under `modified_result_id` as separate immutable artifacts. | (none) | pending-cross-ref | FI §4 (modify not live-tested) |
...
| S1-C82 | §7.2 | state | In-memory dedup set spans current and previous day's files. | (none) | pending-cross-ref | 3A pre-flag |
...
| S1-C93 | §7.6 | behavior | Watch/phone events transmitted over Tailscale; offline events queue locally. | Spec 7 §3.1-3.2 | pending-cross-ref | Depends on Spec 7 |
| S1-C94 | §7.6 | state | `source_event_id` + `timestamp` set on-device; `committed_at` set by log writer. | (none) | pending-cross-ref | Depends on Spec 7 |
...
| S2-C39 | §6.2 | behavior | Reconciliation: config unchanged → no WAL events. | (none) | pending-cross-ref | Reconciliation flow not exercised in FI |
...
| S2-C65 | §9 | invariant | Promotion is always human decision. | (none) | pending-cross-ref | Governance principle (operational) |
...
| S2-C75 | §9.4 | behavior | Promotion flow: orchestrator→job→human edit→restart→reconciliation. | (none) | pending-cross-ref | Operational flow; not exercised |
...
| S2-C77 | §10.1 | behavior | Override cancel/redirect on WAL-1+ → demote 1 (conditional). | Spec 5 §5 | pending-cross-ref | 3A pre-flag |
...
| S2-C79 | §10.1 | behavior | Model change → WAL-0. | (none) | pending-cross-ref | No model change event tested |
| S2-C80 | §10.1 | behavior | WAL-3 approval <99%/90d → WAL-2. | (none) | pending-cross-ref | No WAL-3 cap active |
...
| S3-C4 | §2.1 | structure | `retrieved_candidates` includes excluded w/ reasons. | (none) | pending-cross-ref | Runtime shape detail; not in FI |
| S3-C5 | §2.1 | structure | `eligible_context_fields` post-gate subset. | (none) | pending-cross-ref | Same |
| S3-C6 | §2.1 | invariant | `effective_packaging_level = min(governing WAL, context.package WAL)`. | (none) | pending-cross-ref | Not exercised in FI |
| S3-C7 | §3 | behavior | Three stacked gates: shareability, confidence, token budget. | (none) | pending-cross-ref | Gating not exercised in FI (no memory retrieval path tested) |
| S3-C8 | §3.1 | invariant | `local_only` objects excluded absolutely from cloud. | (none) | pending-cross-ref | No retrieval path tested |
| S3-C9 | §3.1 | state | Default shareability `local_only`. | (none) | pending-cross-ref | Same |
| S3-C10 | §3.2 | state | Below 0.3 confidence → excluded. | (none) | pending-cross-ref | Same |
| S3-C11 | §3.2 | state | Default confidence thresholds per source type. | (none) | pending-cross-ref | 3A pre-flag |
| S3-C12 | §3.3 | state | Token budget three pools. | (none) | pending-cross-ref | Same |
| S3-C13 | §3.3 | behavior | Highest-confidence first; whole objects only. | (none) | pending-cross-ref | Same |
...
| S3-C28 | §5 | behavior | Mixed-content → multiple strip_detail events; field_id in multiple arrays. | (none) | pending-cross-ref | Integration detail; not in FI |
| S3-C29 | §6 | state | WAL-0 context rules (confidence 0.3, 40% memory). | (none) | pending-cross-ref | Not exercised |
| S3-C30 | §6 | state | WAL-1 (50% memory, prompt template). | (none) | pending-cross-ref | Same |
| S3-C31 | §6 | state | WAL-2 (confidence 0.2, 60% memory, exceptions). | (none) | pending-cross-ref | 3A pre-flag |
...
| S4-C18 | §4 | behavior | Check 8 — Payload hash mismatch → policy_violation. | (none) | pending-cross-ref | Not probed |
| S4-C19 | §4 | behavior | Check 9 — Sensitivity scan → context.strip_detail. | (none) | pending-cross-ref | Not probed; egress-gateway emits no audit events per FI §2.2 |
| S4-C20 | §4 | behavior | Check 10 — Auth resolution → policy_violation. | (none) | pending-cross-ref | Not probed |
...
| S4-C22 | §4 | invariant | Only check 9 emits `context.strip_detail`. | (none) | pending-cross-ref | 3A pre-flag |
...
| S4-C25 | §4 | behavior | Gateway re-estimates tokens. | (none) | pending-cross-ref | Not probed |
...
| S4-C28 | §5 | behavior | `route.multi` same package multiple dispatches. | (none) | pending-cross-ref | Not probed |
| S4-C29 | §5 | behavior | Suspended providers excluded; all suspended = blocked. | (none) | pending-cross-ref | Not probed |
| S4-C30 | §5 | invariant | Sensitivity decisions made once, applied uniformly. | (none) | pending-cross-ref | Not probed |
...
| S5-C2 | §1 | invariant | Human decides; DRNT complies+records. | (none) | pending-cross-ref | Governance principle |
...
| S5-C9 | §3 | structure | Successor gets own event chain. | (none) | pending-cross-ref | Not live-tested |
...
| S5-C15 | §5 | behavior | Post-dispatch user gets informational notification. | (none) | pending-cross-ref | UI behavior outside gateway repo |
...
| S5-C17 | §6 | behavior | Cancel after sentinel → no demotion. | (none) | pending-cross-ref | FI §4 (sentinel demotion suppression not live-tested) |
| S5-C18 | §6 | behavior | Redirect viable → demote 1. | (none) | pending-cross-ref | FI §4 (redirect not live-tested) |
| S5-C19 | §6 | behavior | Redirect after sentinel → no demotion. | (none) | pending-cross-ref | Same |
| S5-C20 | §6 | behavior | modify → no demote; score 0.5. | (none) | pending-cross-ref | Not live-tested |
| S5-C21 | §6 | behavior | escalate → no demote; score 0.0. | (none) | pending-cross-ref | Not live-tested |
...
| S5-C23 | §7 | structure | `human.reviewed(modified)` fields (modified_result_id, derived_from, hash). | Spec 1 §4.5 | pending-cross-ref | Modify not live-tested |
| S5-C24 | §7 | invariant | Both artifacts in results store; both hashes in log. | (none) | pending-cross-ref | Same |
| S5-C25 | §8 | behavior | WAL-1 memory write on_accept only. | Spec 2 §5.2 | pending-cross-ref | Depends on Spec 2 §5.2 |
| S5-C26 | §8 | behavior | WAL-2 memory write on delivery; escalate/modify/revoke semantics. | (none) | pending-cross-ref | 3A pre-flag |
| S5-C27 | §8 | invariant | Memory objects never deleted. | (none) | pending-cross-ref | No memory.write exercised |
...
| S5-C29 | §9 | behavior | Score mapping per override-in-window type. | (none) | pending-cross-ref | Not live-tested |
| S5-C30 | §10 | structure | Watch: dismiss=cancel; no redirect/modify/escalate. | (none) | pending-cross-ref | Watch app not in repo |
| S5-C31 | §10 | structure | Phone: all four overrides. | (none) | pending-cross-ref | Phone app not in repo |
| S5-C32 | §10 | behavior | Offline queue; retroactive cancel on delivered → revoked(offline_retroactive). | Spec 7 §3.2.1 | pending-cross-ref | 3A pre-flag |
...
| S5-C35 | §11 | behavior | During packaging: Packager stops. | (none) | pending-cross-ref | Not probed |
| S5-C36 | §11 | behavior | During gateway: abort if not transmitted. | (none) | pending-cross-ref | Not probed |
...
| S5-C39 | §13 | structure | Modify-during-review event sequence. | (none) | pending-cross-ref | Not live-tested |
| S5-C40 | §14 | structure | Post-delivery escalate event sequence. | (none) | pending-cross-ref | Not live-tested |
...
| S6-C4 | §2 | invariant | Three layers, none substitutes for others. | (none) | pending-cross-ref | Architectural principle |
...
| S6-C15 | §2.3 | state | Worker execution defaults deny at endpoint, pass at content (for approved endpoints). | §3.4 | pending-cross-ref | 3A pre-flag |
...
| S6-C17 | §2.4 | invariant | Runtime isolation = ceiling. | (none) | pending-cross-ref | Architectural |
...
| S6-C20 | §2.4 | invariant | Structural privacy applies to all outbound. | (none) | pending-cross-ref | Architectural |
| S6-C21 | §2.4 | invariant | No layer trusts another (defense in depth). | (none) | pending-cross-ref | Architectural |
...
| S6-C41 | §3.3 | behavior | Expired approvals require re-approval. | (none) | pending-cross-ref | No session approval code |
...
| S6-C43 | §3.3 | structure | Session approvals attach to job context. | (none) | pending-cross-ref | No session approval storage code |
...
| S6-C46 | §3.4 | behavior | Check 3 unrecognized pass. | (none) | pending-cross-ref | 3A pre-flag |
...
| S6-C52 | §4.1 | structure | Completion contract fields. | (none) | pending-cross-ref | No completion contract code found |
...
| S6-C62 | §4.3 | behavior | If access not in manifest → job fails; operator updates. | (none) | pending-cross-ref | Not exercised |
...
| S6-C70 | §6 | invariant | Every skill treated as hostile until proven. | (none) | pending-cross-ref | Principle |
| S6-C71 | §6.1 | invariant | Skills installed from outside sandbox only. | (none) | pending-cross-ref | Operational |
| S6-C72 | §6.1 | behavior | Install sequence (fetch/verify/declare/upload/register/recreate). | (none) | pending-cross-ref | Operational |
| S6-C73 | §6.1 | behavior | V1 verification checks. | (none) | pending-cross-ref | 3A pre-flag |
| S6-C74 | §6.2 | structure | Skill permission manifest fields. | (none) | pending-cross-ref | No skill permission manifest code found |
| S6-C75 | §6.2 | invariant | Skill can't access beyond declaration. | (none) | pending-cross-ref | Same |
| S6-C76 | §6.3 | state | Skill origin trust posture. | (none) | pending-cross-ref | Same |
...
| S6-C88 | §7.1 | structure | `agent_type_id` + `agent_instance_id` are payload fields. | (none) | pending-cross-ref | No event builders |
...
| S6-C95 | §7.4 | invariant | `job_id` non-null linked to job trail; sentinel prevents demotion. | (none) | pending-cross-ref | 3A pre-flag |
...
| S6-C100 | §8.3 | behavior | Permanent approvals require manifest edit. | (none) | pending-cross-ref | No session approval flow |
...
| S6-C105 | §9.5 | behavior | Successor runs under own completion contracts. | (none) | pending-cross-ref | Not live-tested |
| S6-C106 | §10 | structure | L2 reads `worker.*` events. | (none) | pending-cross-ref | No L2 code path exercised |
...
| S6-C109 | §10 | invariant | L2 does not gate on worker events. | (none) | pending-cross-ref | No L2 |
...
| S6-C113 | §12.2 | state | V1 out-of-scope list. | (none) | pending-cross-ref | Descriptive |
| S6-C114 | §12.3 | state | Designed-for-but-deferred: SMC + Solid Protocol. | (none) | pending-cross-ref | Descriptive |
...
| S7-C4 | §1 | invariant | MacBook = fallback, not primary. | §5 | pending-cross-ref | No MacBook config |
...
| S7-C7 | §1 | structure | iPhone via Tailscale. | (none) | pending-cross-ref | iPhone app not in repo |
| S7-C8 | §2 | structure | Watch→iPhone via WatchConnectivity. | (none) | pending-cross-ref | iOS layer |
| S7-C9 | §2 | structure | iPhone→Hub Tailscale + /health probe 5s/30s. | (none) | pending-cross-ref | iOS |
| S7-C10 | §2 | structure | Hub→Cloud per-route health probes. | Spec 4 §6 | pending-cross-ref | Depends on Spec 4 §6 |
| S7-C11 | §2 | structure | Hub→iPhone WebSocket heartbeat 10s/30s. | (none) | pending-cross-ref | iOS + WS detail |
| S7-C12 | §2 | structure | iPhone→Watch WatchConnectivity. | (none) | pending-cross-ref | iOS |
...
| S7-C14 | §2 | behavior | iPhone treats non-200 as unavailable. | (none) | pending-cross-ref | iOS |
| S7-C15 | §2.1 | behavior | Asymmetric thresholds: 2 failures down, 3 successes up. | (none) | pending-cross-ref | iOS |
| S7-C16 | §2.1 | structure | "Hub Slow" degraded indicator. | (none) | pending-cross-ref | iOS UX |
| S7-C17 | §2.1 | invariant | Asymmetric hysteresis prevents thundering herd. | (none) | pending-cross-ref | iOS |
| S7-C18 | §2.2 | invariant | APNs best-effort. | (none) | pending-cross-ref | iOS |
| S7-C19 | §2.2 | invariant | Correctness path = polling /jobs/pending + WebSocket. | (none) | pending-cross-ref | No /jobs/pending in FI |
| S7-C20 | §2.2 | invariant | Never depend on push. | (none) | pending-cross-ref | Principle |
...
| S7-C22 | §3.1 | state | Watch queue cap 10; oldest dropped; haptic on drop. | (none) | pending-cross-ref | Watch |
| S7-C23 | §3.1 | behavior | On reactivation, queued forwarded in order. | (none) | pending-cross-ref | Watch |
| S7-C24 | §3.1 | structure | system.connectivity on Watch includes dropped_request_count. | (none) | pending-cross-ref | Watch |
| S7-C25 | §3.2 | structure | iPhone→Hub transitions to Unavailable after 2 failures (60s). | §2.1 | pending-cross-ref | iOS |
| S7-C26 | §3.2 | behavior | Hub unavailable: new requests queued locally; "Queued" badge. | (none) | pending-cross-ref | iOS |
| S7-C27 | §3.2 | state | iPhone queue cap 50; 51st rejected. | (none) | pending-cross-ref | iOS |
| S7-C28 | §3.2 | invariant | Queued persisted to Core Data. | (none) | pending-cross-ref | iOS |
| S7-C29 | §3.2 | invariant | Local storage protection class. | §16 | pending-cross-ref | 3A pre-flag |
| S7-C30 | §3.2.1 | invariant | Queued decision carries result_id + response_hash. | (none) | pending-cross-ref | iOS |
| S7-C31 | §3.2.1 | behavior | Hub validates state matches on replay. | (none) | pending-cross-ref | Spec 5/iOS interaction not live-tested |
| S7-C32 | §3.2.1 | behavior | Hub rejects replayed decision with conflict status. | (none) | pending-cross-ref | Same |
| S7-C33 | §3.2.1 | behavior | iPhone surfaces conflict. | (none) | pending-cross-ref | iOS |
| S7-C34 | §3.2.1 | behavior | Queued submitted in batches of 5 w/ 2s delay. | (none) | pending-cross-ref | iOS |
| S7-C35 | §3.2.1 | behavior | Queued decisions after pending submissions in order. | (none) | pending-cross-ref | iOS |
...
| S7-C41 | §3.3 | behavior | Fallback routing to alternative provider. | (none) | pending-cross-ref | Not probed |
| S7-C42 | §3.3 | invariant | WAL-0 model selection pre_action. | Spec 2 §5.1 | pending-cross-ref | Depends on Spec 2 §5.1 |
| S7-C43 | §3.4 | behavior | Hub→iPhone delivery failure: result in store; retry w/ exp backoff. | (none) | pending-cross-ref | Push layer outside repo |
| S7-C44 | §3.4 | invariant | iPhone app polls /jobs/pending on open. | (none) | pending-cross-ref | iOS |
...
| S7-C46 | §3.5 | behavior | iPhone→Watch delivery failure: displayed normally; Watch queued. | (none) | pending-cross-ref | iOS |
...
| S7-C49 | §3.6 | state | Idempotency store TTL ≥ 7 days. | (none) | pending-cross-ref | 3A pre-flag |
...
| S7-C58 | §4 | behavior | Late arrival after re-dispatch logged, doesn't override. | (none) | pending-cross-ref | Race not exercised |
...
| S7-C60 | §5 | structure | MacBook Pro M5 7B fallback L1. | (none) | pending-cross-ref | No MacBook code |
| S7-C61 | §5 | behavior | MacBook activation manual via iPhone settings. | (none) | pending-cross-ref | Same |
| S7-C62 | §5 | behavior | Hub suspension on failover: suspend_processing command. | (none) | pending-cross-ref | Same |
...
| S7-C64 | §5 | invariant | MacBook has no access to Desktop vector DB/audit/results. | (none) | pending-cross-ref | No MacBook code |
| S7-C65 | §5 | invariant | MacBook WAL state does not transfer; WAL-0 reset. | (none) | pending-cross-ref | Same |
| S7-C66 | §5 | invariant | MacBook Packager uses default sensitivity rules. | (none) | pending-cross-ref | Same |
| S7-C67 | §5 | invariant | No auto failover; exactly one hub authoritative. | (none) | pending-cross-ref | Governance principle |
| S7-C68 | §5 | behavior | Return to primary: V1 does not merge logs. | (none) | pending-cross-ref | No MacBook |
...
| S7-C74 | §7 | invariant | Jobs survive everything except simultaneous storage destruction. | (none) | pending-cross-ref | Boundary scenario |
...
| S7-C76 | §7 | invariant | No auto hub failover. | (none) | pending-cross-ref | Governance |
| S7-C77 | §7 | invariant | Push advisory, polling authoritative. | (none) | pending-cross-ref | iOS |
| S7-C78 | §9.1 | state | Decay windows: WAL-0 none; WAL-1 90d/10; WAL-2 60d/25; WAL-3 30d/50. | Spec 2 §9 | pending-cross-ref | 3A pre-flag |
...
| S7-C82 | §9.2 | invariant | Decay demotion resets counters. | Spec 2 §10.2 | pending-cross-ref | 3A pre-flag |
...
| S7-C88 | §11 | invariant | Failure-triggered fires before decay in same cycle. | (none) | pending-cross-ref | 3A pre-flag |
...
| S7-C98 | §14 | state | Spec 5 integration: version-stamp validation. | (none) | pending-cross-ref | Not live-tested |
...
| S7-C101 | §16 | invariant | Tailscale deny-by-default ACLs; encrypted transport only. | (none) | pending-cross-ref | Operational Tailscale config |
...
| S7-C103 | §16 | invariant | Ollama least-privilege FS, no ambient creds, restricted network. | Spec 4 §3 | pending-cross-ref | Operational (Ollama container config) |

---

## Deferrals

Pending-cross-ref claims total **154** after taxonomy collapse. The list below groups the subset with explicit single-spec cross-references; the remainder (unprobed behavior, off-device artifacts such as iOS/Watch/MacBook, governance principles, or 3A-flagged ambiguities unresolved by findings+code) is deferred en bloc and flagged in the per-spec tables. 3C should prioritize the cross-spec-dependent subset since it can be discharged by correlating with other Spec claims already scored here.

**Explicitly cross-spec-dependent (scored pending-cross-ref because another spec must resolve first):**
...
13. **3B conformance reorder (clerical).** 3B output structure originally drafted with Taxonomy as its own top-level section and Summary placed after Claims; reordered per directive to Preamble → Summary → Claims → Deferrals → Anomalies → Handoff, with the 5-state taxonomy folded into Preamble. Section headings renamed to canonical forms (`## Phase 3B scoring summary` → `## Summary`, `## Claims by spec` → `## Claims`, `## Phase 3B anomalies log` → `## Anomalies`, `## Phase 3B deferrals to 3C` → `## Deferrals`, `## Phase 3B Handoff` → `## Handoff`). No claim scores, counts, evidence notes, claim text, or dispositions changed.
...
- Distribution (5-state taxonomy): aligned=234, partial=77, missing=22, contradicted=27, pending-cross-ref=154
- Pending-cross-ref count handed to 3C: 154 (includes 18 3A-flagged ambiguous claims that didn't resolve from findings+code, cross-spec dependencies, unprobed behaviors, and off-device / off-gateway artifacts such as iOS, Watch, MacBook)
- Anomalies logged: 13 (see Anomalies section; item 13 is the clerical conformance reorder note)
