...
| `isolation.seccomp_profile` | Implemented (runtime-verified on 2026-04-18) | `config/seccomp-default.json` exists and runtime enforcement is now verified in the gateway repo by the fixed-contract probe test `tests/test_e2e_v02.py::TestWorkerExecution::test_worker_seccomp_enforcement_blocks_disallowed_syscall`: inside a worker container, `personality(0xFFFFFFFF)` is blocked with `errno=EPERM` while control syscall `getpid()` succeeds. The earlier path-vs-content diagnosis was accurate for the gateway state audited before Phase 3, but was superseded by failed-path result retention in commit `1073535` and runtime probe infrastructure in commit `13a7abb`. Verification is scoped to this blocked-syscall assertion; full profile exhaustiveness is not claimed. |
...
The worker egress proxy, request sanitizer, dedicated `drnt-worker` and `drnt-bus` segments, skill lifecycle, operator TUI, L2 exit audit, and the `worker.*` event family are out of scope for v0.2 and remain on the v0.3+ track. Runtime seccomp enforcement is now verified for the current gateway state via the fixed-contract worker probe added in gateway commit `13a7abb`, with failed-path observability improved by commit `1073535`. The earlier seccomp regression diagnosis applied to the pre-Phase-3 gateway state and is now retained as historical audit trail rather than current status.
