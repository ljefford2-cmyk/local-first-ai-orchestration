...
The [Local-First AI Orchestration](../README.md) reference architecture was designed as a general pattern — bounded local reasoning with structural privacy enforcement — applicable across any environment where AI must operate under privacy constraints, compliance requirements, and human oversight obligations.
...
| [DRNT](personal-drnt/) | Personal AI gateway on a three-device ecosystem (Watch, Phone, Desktop) with local-first orchestration and self-imposed governance | Self-imposed: privacy by architecture (Context Packager), append-only audit log, graduated autonomy (WAL 0–3) — no external regulatory mandate | Seven interface specifications, three governance artifacts (Event Schema, Capability Trust Profile, NemoClaw Governance Overlay) |
